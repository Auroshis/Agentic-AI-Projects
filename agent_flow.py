...
async def jd_extraction_node(state):
    ...
    prompt = f"""Extract the key required skills and 
...
def gap_analysis_node(state):
    worked = set(state["worked_topics"])
...
async def learning_plan_node(state):
    ...
    prompt = f"""You are a learning coach with IT industry experience. Create a revision/learning plan to cover these topics: {missing}""" 
...
async def resume_tuning_node(state):
    ...
    prompt = f"""You are a career coach with IT industry experience. Given the resume below and the fact that the candidate has these strengths: {overlap}, and needs to emphasise them for this role: {state['job_description']}, produce a tuned version of the resume.\n\nResume:\n{resume}"""
